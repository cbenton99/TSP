...
/**
 * Chris Anderson, Chris Benton
 * Algorithms - TSP Project
 */
public class ImprovedAlgorithm {
	...
	/*
	 * Constructs an instance of a TSPAgent and solves the TSP problem provided as a parameter.
	 */
	...
	/*
	 * Solves the TSP problem by applying greedy nearest neighbor, 2-opt heuristic, and
	...
	private void solve() throws Exception {
		...
		for(int i = 0; i < cities.length; i++) {
			...
			while(twoOpt(tour, dt)); // optimize greedy tour
	...
	/*
	 * Constructs a greedy nearest neighbor tour throughout the cities based on a given starting city.
	 * 	start - index of starting city in cities array.
	 * 	cities - array of unique cities in the TSP instance.
	 */
	public int[] getGreedyTour(int start, int[] cities, DistanceTable dt) throws Exception {
		if(start > cities.length) throw new Exception();
		HashSet<Integer> unvisited = new HashSet<Integer>();
		for(int i : cities) unvisited.add(new Integer(i));
	...
	/*
	 * Calculates the total distance to complete a hamiltonian cycle throughout the tour.
	 */
	...
	private static boolean twoOpt(int[] tour, DistanceTable dt) {
		...
		for(int i = 0; i < tour.length - 3; i++) {
			for(int j = i + 2; j < tour.length - 1; j++) {
				double currDist = dt.getDistanceBetween(tour[i], tour[i + 1]) + dt.getDistanceBetween(tour[j], tour[j + 1]);
				double newDist = dt.getDistanceBetween(tour[i], tour[j]) + dt.getDistanceBetween(tour[i + 1], tour[j + 1]);
				if(currDist > newDist) {
	...
	/*
	 * Reverses the path between idx1 and idx2; used in conjunction with twoOpt heuristic.
	 * 	idx1 - the lower index going to a higher index
	 * 	idx2 - the higher index going to a lower index
	 */
	...
	/*
	 * Uses simulated annealing local search technique to find a solution to the TSP problem.  Returns
	 * a new path if the cost is lower than the input, or returns the input path if a lower cost was
	...
	public static int[] simulatedAnnealing(int[] input, DistanceTable dt, double temp, double decayRate) {
		...
		if(currDist > getTourDist(input, dt)) return input;
		else return currPath;
	}
	
	
	/*
	 * Generates a successor path to be used in conjunction with the simulated annealing algorithm
	 * Calculates two random indices in the path, and forms a new path with the subpath between the
	 * randomly generated indices reversed.  After reversal, the 2-opt heuristic is applied.
	 */
	public static int[] successor(int[] input, DistanceTable dt) {
		...
		while(twoOpt(path, dt));
		return path;
	}
	
	
	/*
	 * Returns the probability simulated annealing accepts the successor cost when worse than current.
	 * 	curr - current path cost (shorter than generated successor cost)
	 * 	succ - successor path cost (longer than current path cost)
	 */
